...
**Ветка:** `ai/fix-platform-date-format`
**Статус:** `completed`
**Коммит:** `73bd1dc`

### Изменения

- `src/components/PlatformDetail.tsx` — удалена локальная `formatTime`; импортирован `formatDateRange` из `@/lib/time`; блок «Время» теперь отображает единую строку «5 сентября, 14:00-14:30» вместо «05.09, 11:00 — 05.09, 14:00». Условие отображения блока — `(dateRange || platform.location)`.
- `tests/components/PlatformDetail.test.tsx` (новый) — 3 теста: единый формат даты с названием месяца; блок «Время» скрыт при невалидных датах; «Место» отображается при невалидных датах.

### Проверки

- `npm run lint` — `passed`
- `npx tsc --noEmit` — `passed`
- `npm test` — `passed` (180 тестов)
- `npm run build` — `passed`

### Для проверки пользователем

- Экран площадки: дата отображается как «5 сентября, 14:00-14:30» (единая строка с названием месяца), а не «05.09, 11:00 — 05.09, 14:00».
